...
public class Timer {
	...
	public String toString() {
		if (timerThread.isAlive())
			return String.format("%s ms timer (%s/%s ticks)", interval, tickCount, count > TIMER_INFINITE ? count : "infinite");
		else
			return String.format("%s ms timer (%s ticks)", interval, count > TIMER_INFINITE ? count : "infinite");
	}
